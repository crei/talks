...
\begin{frame}{Lessons Learnt so Far}
  \begin{itemize}
    ...
    \item Requiring \texttt{[a, b, c].get.Injective} is much easier to deal with than
       $a \neq b \land b \neq c \land a \neq c$.
    \item Loops are what is tricky
  \end{itemize}
\end{frame}

% -------------------------------------------------------
\begin{frame}{Computations on Structured Data}
  \begin{itemize}
...
def sat :=
  to_arg SatInput 2 0 ;
    ...
    to_arg SatInput 1 0
  all_list Clause 0 2 -- check that for all clauses on tape 0
    (any_list Literal 0 2 -- there is some literal
      (case Literal 0
        (contains Var 0 1 2)
