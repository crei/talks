...
\begin{frame}{Space-efficient computations}
  \begin{itemize}
  \item Obvious: $\mathsf{DTIME}(T) \subseteq \mathsf{DSPACE}(T)$
  \item Hopcroft--Paul--Valiant (1975): $\subseteq \mathsf{DSPACE}(T / \log T)$
  \item \textbf{Ryan Williams (2025):} $\subseteq \mathsf{DSPACE}\!\left(\sqrt{T}\cdot\mathrm{polylog}(T)\right)$
  \item Key ingredient: encode many values in the same memory cells using polynomials over finite fields
    and erase anything that is possible to re-compute in the future (Cook-Mertz 2023).
  \end{itemize}
...
\begin{frame}{Personal Perspective}
  \begin{itemize}
    \item I learnt about Williams' result and Lean roughly at the same time.
    \item Let's formalize the result before someone else does!
    \item I had no idea.
    \item First obstacle: cannot even \emph{state} the theorem.
    \item Complexity theory is not formalized at all in Mathlib / Cslib
    \item Before Gäher-Kunze-2021, complexity theory was not formalized (down to the machine model) in any proof assistant.
  \end{itemize}
...
\begin{frame}{State of the Art}
  \begin{itemize}
    \item Generally seen as very difficult to formalize, several discussions in Zulip.
...
\begin{frame}[fragile]{TM DSL}
  The DSL evaluates to regular Turing machines but operates on lists of words instead of tapes.
...
\begin{frame}{Next Tasks}
  \begin{itemize}
    \item For sub-linear space, we probably cannot stay with stacks but need to use arrays.
    \item Loops are not automatic but OK, is there a better way?
